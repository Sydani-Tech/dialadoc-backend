<?php
    ...
    if (isset($_SESSION['Admin_Name'])) {
        ...
        function gtStudTotalScore ($studId, $ClsId, $TrmId, $YrId) {
            global $db;
            $gtTtScr_StAvr = $db->getRow('SELECT SUM(`Total_Score`) AS Total_Score, 
            COUNT(*) AS Noof_Sub FROM `scores` WHERE `Students_id` = ? 
            AND `Students_Classess_id` = ? AND `Terms_id` = ? AND `Terms_Years_id` = ?', 
            [$studId, $ClsId, $TrmId, $YrId]);
            return $gtTtScr_StAvr;
        }
        function getStuTatalScrTable ($studId, $ClsId, $TrmId, $YrId) {
            global $db;
            $gtTtscrTable = $db->getRow('SELECT * FROM `totalscore` 
            WHERE Student_id = ? AND Class_id = ? AND Term_id = ? AND Year_id = ?',
            [$studId, $ClsId, $TrmId, $YrId]);
            return $gtTtscrTable;
        }
        function ins_UpdateStuTtlAvrScr ($studId, $ClsId, $TrmId, $YrId) {
            global $db;
            if(empty(getStuTatalScrTable($studId, $ClsId, $TrmId, $YrId))) {
                $gtStuTtl = gtStudTotalScore($studId, $ClsId, $TrmId, $YrId);
                $StuTtScr = $gtStuTtl['Total_Score'];
                $StuAvr = $gtStuTtl['Total_Score'] / $gtStuTtl['Noof_Sub'];

                $insTtscr = $db->insertRow('INSERT INTO `totalscore` (`total_all_scores`, `Student_average`, 
                `Student_id`, `Class_id`, `Term_id`, `Year_id`) VALUES (?, ?, ?, ?, ?, ?)',
                [$StuTtScr, $StuAvr, $studId, $ClsId, $TrmId, $YrId]);
            } else {
                $gtStuTtl = gtStudTotalScore($studId, $ClsId, $TrmId, $YrId);
                $StuTtScr = $gtStuTtl['Total_Score'];
                $StuAvr = $gtStuTtl['Total_Score'] / $gtStuTtl['Noof_Sub'];

                $insTtscr = $db->insertRow('UPDATE `totalscore` SET `total_all_scores`= ?,`Student_average`= ? 
                WHERE `Student_id` = ? AND `Class_id` = ? AND `Term_id` = ? AND `Year_id` = ?',
                [$StuTtScr, $StuAvr, $studId, $ClsId, $TrmId, $YrId]);
            }
        }
        ...
        function gtClassStu ($clsId, $subjId, $TrmId, $YrId) {
            ...
            foreach ($gtStu as $val) {
                $stScore = gtStudScores($val['id'], $clsId, $TrmId, $YrId, $subjId);
                // print_r($stScore);
                // exit('Exit Code here...');
                if ($stScore) {
                    $out .= '
        ...
        if (isset($_POST['addStudScoreId'])) {
            ...
            ins_UpdateStuTtlAvrScr($studId, $ClsId, $TrmId, $YrId);
            } else {
